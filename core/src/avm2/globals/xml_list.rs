...
use crate::{
    avm2::{
        ...
        Activation, Error, Object, TObject, Value, QName,
    },
    avm2_stub_method,
};
        ...
        Value::Object(o) => {
            if let Some(qname) = o.as_qname_object() {
                *qname.qname().unwrap()
                ...
                QName::new(activation.avm2().public_namespace, name.coerce_to_string(activation)?)
            }

        }
        _ => QName::new(activation.avm2().public_namespace, name.coerce_to_string(activation)?),
    };
