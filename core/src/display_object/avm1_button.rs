...
impl<'gc> Avm1Button<'gc> {
    ...
    fn set_state(
        mut self,
        context: &mut crate::context::UpdateContext<'gc>,
        state: ButtonState,
    ) {
...
impl<'gc> TInteractiveObject<'gc> for Avm1Button<'gc> {
    ...
    fn event_dispatch(
        ...
        let (update_state, new_state) = if is_enabled {
            ...
            if InteractiveObject::option_ptr_eq(self.as_interactive(), context.mouse_hovered_object) {
...
impl<'gc> Avm1ButtonData<'gc> {
    fn play_sound(&self, context: &mut UpdateContext<'gc>, sound: Option<&swf::ButtonSound>) {
